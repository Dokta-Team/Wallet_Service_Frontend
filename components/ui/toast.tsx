...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed top-[10%] left-[50%] z-[100] flex justify-center items-center p-4 w-[90%] md:w-[30%] lg:w-[30%] translate-x-[-50%] translate-y-[-50%]",
      className
    )}
    {...props}
  />
...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border border-slate-200 p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full dark:border-slate-800",
  {
    variants: {
      variant: {
        default:
          "bg-emerald-300 rounded-lg text-emerald-600 border-none text-base font-bold leading-tight justify-center items-center gap-2.5 inline-flex text-center",
        destructive:
          "w-full bg-rose-400 rounded-lg text-red-600 border-none text-base font-bold leading-tight justify-center items-center gap-2.5 inline-flex text-center",
        copied:
          "w-full h-[59px] px-6 py-4 bg-blue-200 rounded-lg justify-center items-center gap-2.5 inline-flex text-blue-900 text-base font-normal leading-7  text-center",
      },
    },
    defaultVariants: {
      variant: "default",
